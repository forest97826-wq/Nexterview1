...
> 不只是刷题工具，也不只是一次性模拟面试。  
> Nexterview 把专项训练、简历面试、JD 备面、实时 Copilot 与录音复盘串成一个持续进化的技术面试闭环。
...
Nexterview 的目标不是“再生成一组题”，也不是只做一轮模拟面试，而是构建一个**覆盖备面、模拟、实战辅助与复盘的持续演化闭环**：
...
![Nexterview personalized question generation engine](images/question-generation-engine.png)

Nexterview 不从固定题库随机抽题，而是融合三层信息后生成下一轮问题：
...
![Nexterview training evaluation profile update loop](images/training-loop.png)
...
![Nexterview system architecture overview](images/system-architecture.png)

Nexterview 不是单页 Demo，而是完整系统：
...
[![Star History Chart](https://api.star-history.com/svg?repos=AnnaSuSu/Nexterview&type=Date)](https://star-history.com/#AnnaSuSu/Nexterview&Date)
