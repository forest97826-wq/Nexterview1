      ...
      {yLabels.map((v) => {
        ...
        return (
          <g key={v}>
            ...
            <text x={PAD.left - 8} y={y + 4} textAnchor="end" fill="var(--muted-foreground)" fontSize={11}>{v}</text>
          </g>
        );
      })}
      {xIndices.map((i) => (
        <text key={i} x={points[i].x} y={H - 6} textAnchor="middle" fill="var(--muted-foreground)" fontSize={11}>
    ...
    <div className="flex-1 px-4 py-8 md:px-6 md:py-10 max-w-3xl mx-auto w-full">
      ...
      {Object.keys(profile.topic_mastery || {}).length > 0 && (
        <div className="mb-7 animate-fade-in-up [animation-delay:0.15s]">
          <div className="text-base font-semibold mb-3">领域掌握度</div>
